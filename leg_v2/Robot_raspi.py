...
class RobotController:
    ...
    def setup_controllers(self):
        ...
        trajectory_controller = WalkTrajectoryController(
            TrajectoryType.WALK, self.config.gait_config.get_default_gait_params()
        )
        linear_trajectory_controller = TwerkTrajectoryController(
            TrajectoryType.LINEAR, self.config.gait_config.get_default_gait_params()
        )
        ...
        # Sit controller
        self.controllers[RobotMode.SIT] = SitController(self.motor_handler, self.config)
        ...
        # Stand controller
        self.controllers[RobotMode.UP] = UpController(self.motor_handler, self.config)

        self.controllers[RobotMode.TWERK] = TwerkController(
            linear_trajectory_controller,
            self.motor_handler,
            self.imu_controller,
            self.config,
        )
    ...
    def initial_setup(self):
        ...
        for leg in self.config.legs:
            for motor_id in [leg.upper_id, leg.lower_id, leg.inner_id]:
                self.motor_handler.set_speed(
                    motor_id, self.config.default_motor_speed // 2
                )  # Slower for sitting
...
class ExcitedController(Controller):
    ...
    def initial_setup(self):
        ...
        for leg in self.config.legs:
            for motor_id in [leg.upper_id, leg.lower_id, leg.inner_id]:
                self.motor_handler.set_speed(
                    motor_id, self.config.default_motor_speed
                )  # Normal speed for excitement
                self.motor_handler.set_torque(motor_id, 1)
...
class TurnController(Controller):
    ...
    def initial_setup(self):
        ...
        for leg in self.config.legs:
            for motor_id in [leg.upper_id, leg.lower_id, leg.inner_id]:
                self.motor_handler.set_speed(
                    motor_id, self.config.default_motor_speed // 3
                )
                self.motor_handler.set_torque(motor_id, 1)
...
def terminal_input_handler(robot_controller):
    ...
    command_map = {
        ...
        "idle": RobotMode.IDLE,
        "i": RobotMode.IDLE,
        "twerk": RobotMode.TWERK,
        "t": RobotMode.TWERK,
        "down": RobotMode.DOWN,
        "d": RobotMode.DOWN,
        "excited": RobotMode.EXCITED,
        "e": RobotMode.EXCITED,
        "left": RobotMode.TURN_LEFT,
        "l": RobotMode.TURN_LEFT,
        "right": RobotMode.TURN_RIGHT,
        "r": RobotMode.TURN_RIGHT,
        "jump": RobotMode.JUMP,
        "j": RobotMode.JUMP,
    }
...
def main():
    ...
    input_thread = threading.Thread(
        target=terminal_input_handler, args=(robot_controller,), daemon=True
    )
    input_thread.start()
